...
class SILab2Test {
    ...
    void checkEveryBranch() {
        ...
        //5
        assertFalse(SILab2.checkCart(create(new Item(" ", "01111", 500, 0.4f)), 2));

        //6
        exception = assertThrows(RuntimeException.class, () -> SILab2.checkCart(create(new Item("Angel", "0a11b", 20, 0.5f)), 1));
        assertTrue(exception.getMessage().contains("Invalid character in item barcode!"));

        //7
        assertFalse(SILab2.checkCart(create(new Item("Angel", "111111", 500, -1)), 2));

    }

    @Test
    void checkMultipleConditions() {
        assertFalse(SILab2.checkCart(create(new Item("", "11111", 400, 0.5f)), 2));
        assertFalse(SILab2.checkCart(create(new Item("", "11111", 400, 0f)), 2));
        assertTrue(SILab2.checkCart(create(new Item("", "01111", 400, 0.5f)), 200));
        assertFalse(SILab2.checkCart(create(new Item("", "01111", 200, 0.5f)), 2));
    }
}